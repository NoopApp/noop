...
/// (WhoopExportImporter here, WhoopCsvImporter on Android) re-import them losslessly — the
/// round-trip is pinned by WhoopCsvExporterTests parsing the output with the real importer.
///
/// Header strings are byte-identical to a real WHOOP export (see Tests/Resources fixtures).
...
public enum WhoopCsvExporter {
    ...
    /// Locale-proof numbers: integral Doubles print without ".0"; String(Double) uses '.'.
    ...
    /// "wake" == awake). Unusable input → all-nil.
    struct StageMinutes {
    ...
    /// Z1–Z5 percents from zonesJSON ("z1"…"z5" macOS import / "zone1"…"zone5" Android),
    /// or nil when the row carries no usable zone data.
    ...
    /// series: day -> (metricSeries key -> value) for the cycles columns DailyMetric lacks.
    ...
    public static func journalCSV(_ rows: [JournalEntry]) -> String {
        ...
        for e in rows.sorted(by: { ($0.day, $0.question) < ($1.day, $1.question) }) {
            let cols: [String] = [
                ...
                // The importer requires the literal "true" — never prettify to Yes/No.
