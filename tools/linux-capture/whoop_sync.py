...
# --- whoop4 frame helpers (verified offline) ------------------------------------------------------

def _verify_whoop4(frame: bytes) -> bool:
    if len(frame) < 8 or frame[0] != 0xAA:
        return False
    total = (frame[1] | (frame[2] << 8)) + 4
    if total != len(frame) or wf.crc8(bytes(frame[1:3])) != frame[3]:
        return False
    inner = bytes(frame[4:total - 4])
    return wf.crc32(inner) == int.from_bytes(frame[total - 4:total], "little")


def _whoop4_history_end_data(frame: bytes):
    # whoop4: inner @ 4, meta_type @ 6, trim cursor @ 17:25
    if len(frame) < 25 or frame[4] != PACKET_METADATA or frame[6] != META_HISTORY_END:
        return None
    if not _verify_whoop4(frame):
        return None
    return bytes(frame[17:25])


def _whoop4_ack(end_data: bytes, seq: int = 0) -> bytes:
    return wf.build_command_frame(CMD_HISTORICAL_DATA_RESULT, seq=seq, payload=b"\x01" + bytes(end_data))
...
class Family:
    ...
    def __init__(self, model: str):
        ...
        if model == "whoop4":
            ...
            self._end_data = _whoop4_history_end_data
            self._ack = _whoop4_ack
