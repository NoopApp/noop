...
class Whoop4FrameHelpers(unittest.TestCase):
    ...
    def test_verify_roundtrip(self):
        ...
        self.assertTrue(ws._verify_whoop4(f))
        self.assertFalse(ws._verify_whoop4(f[:-1] + b"\x00"))   # corrupt CRC32
    ...
    def test_history_end_data_extracts_trim_cursor(self):
        ...
        self.assertEqual(ws._whoop4_history_end_data(frame), end)

    def test_history_start_is_not_acked(self):
        frame = _make_whoop4_metadata(ws.META_HISTORY_START)
        self.assertIsNone(ws._whoop4_history_end_data(frame))   # only HISTORY_END yields ack data
    ...
    def test_corrupt_history_end_rejected(self):
        ...
        self.assertIsNone(ws._whoop4_history_end_data(bytes(frame)))

    def test_ack_payload_is_0x01_plus_end_data(self):
        end = b"\x11\x22\x33\x44\xAA\xBB\xCC\xDD"
        ack = ws._whoop4_ack(end, seq=50)
        self.assertTrue(ws._verify_whoop4(ack))
