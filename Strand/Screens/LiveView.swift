...
struct LiveView: View {
        ...
        NoopCard(padding: 14) {
            HStack(alignment: .center, spacing: 12) {
                ...
                HStack(spacing: 12) {
                    ...
                    headerStat("Worn", live.worn ? "Yes" : "No")
    ...
    private var rrStrip: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            Text(values.isEmpty ? "Waiting for R-R intervals." : "Recent intervals: " + values.suffix(5).map(String.init).joined(separator: " · ") + " ms")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .lineLimit(1)
                .minimumScaleFactor(0.7)
    ...
    private var signalTrustSummary: String {
        ...
        if activeConnection { return "Live heart rate is flowing; full strap controls need encrypted bond." }
    ...
    private var signalTiles: [SignalTrustTile.Model] {
        [
            ...
            .init(title: "Wear state",
                  value: live.worn ? "On wrist" : "Off wrist",
                  detail: live.worn ? "Eligible for live physiology" : "Wear the strap for scoring",
                  icon: "sensor.tag.radiowaves.forward",
                  tint: live.worn ? StrandPalette.accent : StrandPalette.statusWarning)
        ]
    }
