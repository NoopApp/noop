...
private struct VitalsSection: View {
    ...
    /// The vitals row, built from the most recent imported day, banded against the user's
    ...
    private var vitals: [Vital] {
        let d = repo.today
        let todayKey = d?.day
        // History strictly before the displayed day, oldest→newest; calendar-padded so
        // wear gaps count as missing nights (staleness → population fallback).
        ...
        // Skin temp is bimodal: CSV imports store ABSOLUTE °C, the on-device pipeline a
        // ±°C DEVIATION — partition the history to the displayed value's kind and pick
        // the matching config + population fallback (±0.6 °C mirrors the illness watch).
        ...
        return [
            ...
            Vital(key: "spo2", label: "Blood O₂", unit: "%",
                  ...
                  // Population-only on purpose: an absolute <95% floor is meaningful
                  // regardless of personal baseline (no "spo2" MetricCfg exists).
